...
def store_ranklist(ranklist, filename):
    ...
    for current_case in list(ranklist.keys())[10:]:
        ...
        for doc in docs:
            lines.append(current_case+" Q0 "+doc[0:-4]+" "+str(docs.index(doc)+1)+' '+str(scores[docs.index(doc)])+" "+'para_level_combined'+'\n')
...
def main():
    ...
    s1 = load_pickle(pickle_path, 'P_1')
    s2 = load_pickle(pickle_path, 'CAT_3')
