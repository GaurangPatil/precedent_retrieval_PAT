...
def get_pointing_judgments(mapping, citation_graph):
    ...
    for doc in tqdm(required_docs, desc='Retrieving pointing judgments'):
        cited_by_list = []
    ...
    return cited_by
def get_citation_anchor_text_doc_level(required_ik_docs_are_cited_by,judgment_data, vectorizer_filename, mapping):
    pickle_path = '../data/pickled_files'
    tfidf_vectorizer = load_pickle(pickle_path, vectorizer_filename)
    citation_anchor_text_doc_level_text = defaultdict(int)
    citation_anchor_text_doc_level_vectors = defaultdict(int)
        ...
        for judgment_id in pointing_judgments:
            for para in judgment_data[judgment_id].keys():
                if aila_judgment in judgment_data[judgment_id][para]['sc_judgments_cited']:
                    paras += ' '+ judgment_data[judgment_id][para]['text']
        citation_anchor_text_doc_level_text[mapping[aila_judgment]] = paras
        citation_anchor_text_doc_level_vectors[mapping[aila_judgment]] = tfidf_vectorizer.transform([paras] if len(paras) else ['empty'])
    return citation_anchor_text_doc_level_text, citation_anchor_text_doc_level_vectors
...
def main():
    ...
    for key, (value_str, num) in high_confidence_mapping_aila.items():
        new_key = copy.deepcopy(value_str)
        new_value = copy.deepcopy(key)
        reverse_mapping[new_key] = new_value
    citation_anchor_text_doc_level_text, citation_anchor_text_doc_level_vectors = get_citation_anchor_text_doc_level(required_ik_docs_are_cited_by,
                                                                                                                     judgment_data, 
                                                                                                                      vectorizer_name,
                                                                                                                      reverse_mapping )

    store_as_pickle(pickle_path, 'citation_anchor_text_doc_level_text', citation_anchor_text_doc_level_text)
    store_as_pickle(pickle_path, 'citation_anchor_text_doc_level_vectors',citation_anchor_text_doc_level_vectors)
